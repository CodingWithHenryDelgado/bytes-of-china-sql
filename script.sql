...
SELECT restaurant.name AS restaurant_name,(address.street_number, address.street_name) AS address, restaurant.telephone AS telephone
FROM restaurant
JOIN address
...
JOIN category
  ON category.id = categories_dishes.categories_id
ORDER BY price ASC;
...
SELECT dish.name,
COUNT(categories_dishes.categories_id)
FROM dish, categories_dishes
GROUP BY dish.id, dish.name,categories_dishes.dish_id
HAVING 
  COUNT(categories_dishes.categories_id) > 1 AND categories_dishes.dish_id = dish.id;
